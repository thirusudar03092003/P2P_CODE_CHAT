...
class P2PChatNode:
    ...
    def send_message_to_peers(self, content):
        ...
        # Send to all peers using existing connections
        message_data = json.dumps(message).encode('utf-8')
        for peer_id in list(self.connected_peers):
            try:
                if peer_id in self.peer_connections:
                    self.peer_connections[peer_id].send(message_data)
                else:
                    print(f"\033[91mNo active connection for peer {peer_id}\033[0m")
                    if peer_id in self.connected_peers:
                        self.connected_peers.remove(peer_id)
                    if peer_id in self.peers:
                        del self.peers[peer_id]
    ...
    def stop(self):
        ...
        for peer_id, socket in list(self.peer_connections.items()):
            try:
                socket.close()
            except:
                pass
